...
@section('breadcrumb')
    {{ ucfirst($routePrefix) }} / <a href="{{ route($routePrefix . '.elections.index') }}">Elections</a> / <a href="{{ route($routePrefix . '.elections.positions.index', $election) }}">Positions</a> / <a href="{{ route($routePrefix . '.elections.positions.candidates.index', [$election, $position]) }}">Candidates</a> / <span>Add</span>
@endsection
...
<div class="v-page-header">
    <div>
        ...
        <div class="v-page-header__sub">{{ $election->title }} / {{ $position->name }}</div>
    </div>
...
<div class="v-card" style="max-width:580px;">
    <div class="v-card__body">
        <form method="POST" action="{{ route($routePrefix . '.elections.positions.candidates.store', [$election, $position]) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                ...
                <div class="v-form-group">
                    ...
                    <input id="partylist" class="v-input" type="text" name="partylist" value="{{ old('partylist') }}" placeholder="e.g. Independent, KASAMIS">
